...
class Command(BaseCommand):
    ...
    def add_to_settings(self, app_path):
        ...
        for i, line in enumerate(lines):
            if "LOCAL_INTERNAL_APPS" in line:  # Encontrar onde começa a lista
                local_apps_start = i
        ...
        if not local_apps_found:
            self.stdout.write(self.style.ERROR("A lista LOCAL_INTERNAL_APPS não foi encontrada no settings.py."))
            return
